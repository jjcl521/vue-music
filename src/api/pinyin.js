 ...
 * @param isFirst 是否取首字母
 * @returns {string} 结果
 */
export function getPinYin(str, split = ' ', uppercase = false, isFirst = false) {
  let l2 = str.length;
  let result = "";
  ...
  if (isFirst)
    return result.trim().substring(0, 1);
  return result.trim();
}
...
export function isChinese(str) {
  let reg = new RegExp(/[^\x00-\x80]/g)
  let entryVal = str;
  let cnChar = entryVal.match(reg);
